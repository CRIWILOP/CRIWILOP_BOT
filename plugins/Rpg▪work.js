...
let handler = async (m, { conn, isPrems}) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastwork < 600000) { conn.sendNyanCat(m.chat, `*Estas cansado* y por lo tanto hay que esperar ${msToTime(time - new Date())} para volver a trabajar!`, await (await fetch('https://telegra.ph/file/0056079ce61eb0cbf5768.jpg')).buffer(), '🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀', `Tᴏᴍᴀ́ɴᴅᴏsᴇ ᴜɴ ᴄᴀғᴇᴄɪᴛᴏ☕`, script, m)
  throw false }
  let img = `${pickRandom(global.workimg)}`
  let imgwork = await (await fetch(img)).buffer()
await conn.sendNyanCat(m.chat, `‣ ${pickRandom(global.work)} *${hasil} XP*`, imgwork, '🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀', `Wᴏʀᴋ ʙʏ 🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀👷🏻`, script, m)
...
"Ayudas a moderar el grupo de 🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀 por", "Diseñaste un logo para 🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀 por", 
"Moderaste el grupo cuando 🍀❥៚⍣𝘾𝙍𝙄𝙒𝙄𝙇𝙊𝙋_𝘽𝙊𝙏࿐⛦🍀 no estaba, el pago fue", 
"¡Trabajó lo mejor que pudo en una imprenta que estaba contratando y ganó su bien merecido!", 
"Trabajas como podador de arbustos para *⍣ᴄʀɪᴡɪʟᴏᴘ࿐* y ganas", "La demanda de juegos para dispositivos móviles ha aumentado, por lo que creas un nuevo juego lleno de micro-transacciones. Con tu nuevo juego ganas un total de", 
